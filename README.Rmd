...
The \(\theta_1\) parameter is strongly consentrated at zero, traceplots for beta show convergence for the betat parameter and density for \(I_1\) also indicate evidence that this variable does not contribute much to the regression. The true value for \(\theta_1 = 0.02857143\) which is very small, the posterior mean estimate is `r round(mean(thinnedsample[,"beta[1]"]*thinnedsample[,"Ind[1]"]),6)`. Using the skimr package to see additional results:
